...
export const findErpDisabledProducts = async () => {
  const batchedCodes = disabledCodes
    .map((code) => ({ cod: code.cod.toUpperCase() }))
    .reduce((acc, code, i) => {
  ...
  for (const batch of batchedCodes) {
    writeStdOut(`Fetching batch ${i++} of ${batchedCodes.length}`);
    const batchedErpProducts = await Promise.all(
      batch.map((code) =>
        erp
          .getOneProduct(code.cod || "")
          .then((erpProduct) => {
            if (!erpProduct) {
  ...
  //   missingErpProducts.map((product) => ({
